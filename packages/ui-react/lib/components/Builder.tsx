...
import { QueryResolver } from "@yaqb/query-resolver";
...
export const Builder: React.FC<BuilderProps> = ({ rules, fields }) => {
  const [queryBuilder, setQueryBuilder] = useState(
    new QueryBuilder(rules, fields)
  );

  const addRule = (segment: string) => {
    const segmentField = fields.find(
      (field) => field.metadata?.category === segment
    );
            ...
            {queryBuilder.rules.rules.map((rule: RuleElem, index: number) =>
              "field" in rule ? (
                <CustomRule
                  key={index}
                  title={
                    fields.find((field) => field.field === rule.field)?.metadata
                      ?.category
                  }
                  ruleIndex={index}
                  rule={rule as Rule}
